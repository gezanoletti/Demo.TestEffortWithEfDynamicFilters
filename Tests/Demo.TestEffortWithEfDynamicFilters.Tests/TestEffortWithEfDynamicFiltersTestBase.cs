...
{
    public abstract class TestEffortWithEfDynamicFiltersTestBase : AbpIntegratedTestBase<TestEffortWithEfDynamicFiltersTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TestEffortWithEfDynamicFiltersTestBase()
        {
        ...
        protected void UsingDbContext(Action<TestEffortWithEfDynamicFiltersDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestEffortWithEfDynamicFiltersDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestEffortWithEfDynamicFiltersDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestEffortWithEfDynamicFiltersDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestEffortWithEfDynamicFiltersDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEffortWithEfDynamicFiltersDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestEffortWithEfDynamicFiltersDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestEffortWithEfDynamicFiltersDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestEffortWithEfDynamicFiltersDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestEffortWithEfDynamicFiltersDbContext, Task<T>> func)
        {
            T result;
